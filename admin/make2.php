...
if($_GET['action'] == "sort"){
	...
	$sort = $DB->fetch_array($query);
	    ...
	    $goon=1;
     echo "<font color=red><b>正在生成文章分类的Html页,请等待......</b></font><br><br><span id=showImport></span>";
...
echo "<script>function onDownloadDone(downDate){showImport.innerHTML=downDate}oDownload.startDownload('makesort.php?total=".$total."&sortid=".$row['sortid']."&sortdir=".$row['sortdir']."&i=".$i."',onDownloadDone)</script>";
    ...
    if($goon){
        ...
        echo "<b>操作成功！共生成分类<font color=red>".$maketotal."</font>个，页面<font color=red>".($page+1)."</font>个 <a href=./article.php?action=make>[返回]</a></b>";
    }
}
//生成首页
