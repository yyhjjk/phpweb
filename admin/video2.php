...
$tomtq = explode("<table width=\"98%\" height=\"100%\" border=\"0\" align=\"center\" cellpadding=\"0\" cellspacing=\"0\">",$file);
$tomtq = explode("</table>",$tomtq[1]);
$tomtq= $tomtq[0];

$title=explode("<strong>",$tomtq);
$title=explode("</strong>",$title[1]);
$title=$title[0];

$content=explode("<p align=left>",$tomtq);
$contentr=explode("</p>",$content[1]);
$content=$contentr[0];
$content=str_replace("http://www.13mv.com/play.asp","/play.php",$content);


$tomtq=$title."<br>".$content;
...
if ($_GET['action']=="add")
{
    ...
    $cpforms->formheader(array('title'=>'添加文章',
                                'name'=>'form',
                                'action'=>'http://86enjoy.com/yyhadminxy/articleoperate1.php',
                                  'extra'=>"onSubmit=\"return ProcessArticle()\" onkeydown='with(window.event)if(keyCode==13&&ctrlKey)submit()'"));
